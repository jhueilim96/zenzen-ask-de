...
from ...resources.core import TypesenseSearchIndexResource, Neo4jGraphResource
...
def update_webpage_index(
    ...
    load_script_path = (
        Path().cwd()
        / "ani_dagster/assets/search_index/cypher_query/ser-webpage-retrieve.cypher"
    )
    count_script_path = (
        Path().cwd()
        / "ani_dagster/assets/search_index/cypher_query/ser-webpage-count.cypher"
    )
    ...
    for skip_count in range(0, entity_count, BATCH_SIZE):
        res = neo4jgraph.query(
            cypher_load, {"BATCH_SIZE": BATCH_SIZE, "skip_count": skip_count}
        )
        index_input_data = [doc.data() for doc in res]
        index_res = client.collections[SEARCH_INDEX_NAME].documents.import_(
            index_input_data, {"action": "upsert"}
        )
...
def anime_in_entity_index(
    ...
    load_script_path = (
        Path().cwd() / f"ani_dagster/assets/search_index/cypher_query/{label}.cypher"
    )
    with open(str(load_script_path), "r") as fp:
        cypher_load = fp.read()
